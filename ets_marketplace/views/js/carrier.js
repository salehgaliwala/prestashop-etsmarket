...
function onFinishCallback(obj, context)
{
	...
	$.ajax({
		...
		async: false,
		dataType: 'json',
		data : data ,//$('#carrier_wizard .stepContainer .content form').serialize() + '&action=finish_step&ajax=1&step_number='+context.fromStep,
		contentType: false,
        processData: false,
...
function bind_inputs()
{
	...
	$('input[name="shipping_method"]').off('click').on('click', function() {
		$('input[name="shipping_method"]').prop('checked',false).parent().removeClass('checked'); //s
		$(this).prop('checked',true).parent().addClass('checked'); //s
		$.ajax({
			type:"POST",
			url : validate_url,
			async: false,
...
function showFees()
{
	$('tr.range_inf td, tr.range_sup td, tr.fees_all td, tr.fees td').each(function () {
		if ($(this).index() >= 2)
		{
			...
			//if ($(tr).index() > 2 && $(tr).find('td:eq(1) input').is(':checked') && validate || !$(tr).hasClass('range_sup') || !$(tr).hasClass('range_inf'))
				//$(this).find('div.input-group input:text').removeAttr('disabled');
